...
- **Launch checklist + cutover comms (D-10-19..21):**
  - **Antoine (you) owns partner comms.** Direct involvement, not delegated to Thomas. The deploy-ovh.md runbook is written assuming Antoine-context (technical voice, not generic operator). Antoine drafts the partner email (technical content: invitation URL flow, password setup steps, "Leasetic Matrice has moved to a hosted version" framing). Thomas reviewed for tone/relationship sensitivity if needed.
...
- **D-10-09 (Email-pattern discrimination, no schema change):** Test partners are tagged via email pattern: `<anything>@test.leasetic.com`. The `@test.leasetic.com` domain is reserved for test partners only; production partners use real Leasetic-network domains. No `is_test` column added to schema. CUT-04's literal "is_test=true" wording is satisfied by the equivalent email-pattern predicate. **REQUIREMENTS.md update:** add a note to CUT-04 that the discriminator is realized via email pattern (planner can do this inline or as a separate REQ doc edit).
