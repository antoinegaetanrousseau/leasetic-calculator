...
> **📦 ARCHIVED 2026-05-30 at v1.4 milestone close.** Final status: 19/19 active
> requirements satisfied; BRAND-01/02/03 (teal rebrand) descoped to Future Requirements.
> Full audit: `milestones/v1.4-MILESTONE-AUDIT.md`. Milestone report:
> `reports/MILESTONE_SUMMARY-v1.4.md`. This is a frozen snapshot — the working
> `REQUIREMENTS.md` was removed at close and is regenerated fresh for the next milestone.
...
- [~] **BRAND-02** *(descoped)*: The Leasetic **logo green** and the **semantic success green** (the "Actif" status pill, "activé le compte" activity entries) are left unchanged.
...
- [x] **COPY-01**: The admin-home "Références LC" card title and the corresponding page heading read **"Toutes les propositions"** (FR) / equivalent EN. The `/[adminSegment]/lc-references` route is unchanged (display label only).
- [x] **COPY-02**: The admin-home "Coefficients & commission" card title reads **"Coefficients & Commissions"**.
- [x] **COPY-03**: The admin-home "Dernière modif. coeffs" stat label reads **"Dernière Modif Coef"**.
- [x] **COPY-04**: All label changes ship FR + EN dictionary entries; the compile-time `_EnHasAllFrKeys` parity proof stays green.

### Component Fix (UIFIX)

- [x] **UIFIX-01**: The proposal status pill (e.g. "Actif", "Brouillon", "Expirée") sizes to its text content (hugs content) rather than rendering at a fixed width — verified across all status variants and both languages.
