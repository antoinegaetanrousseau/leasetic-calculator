...
| `src/components/ui/alert-dialog.tsx` | `AlertDialogContent`'s root reads `rounded-container`, not upstream's `rounded-4xl`. | UIC-04 / Phase 31.1-04 (D-01, OPEN-A): container surfaces read `--radius-container`; `rounded-4xl` is the CONTROL tier, shared with Input/Button/Select. Upstream couples a destructive confirm dialog's corner to the control tier, which 31.1-04 deliberately broke. **Measured to recur:** `npx shadcn add @reui/solution-users-2 -o` on 2026-09-03 clobbered this back to `rounded-4xl` and was restored by hand. `tests/container-radius.test.ts` catches it, but only after the fact. Added 2026-09-03 (34-03). |
| `src/components/blocks/solution-crm-5/components/activity-timeline.tsx` | The empty-state icon keeps `strokeWidth={1.9}` (number), not upstream's current `strokeWidth="1.9"` (string). | `HugeiconsIcon` types `strokeWidth` as `number`, so upstream's string form is `TS2322: Type 'string' is not assignable to type 'number'` and fails `npm run typecheck` outright. **Measured to recur:** `npx shadcn add @reui/solution-crm-5 -o` on 2026-09-03 reintroduced the string form; it was the block refresh's ONLY net change. Every other block in this repo passes the number form. Added 2026-09-03 (34-03). |
| `src/components/ui/dialog.tsx` | The icon-only close button's `sr-only` label reads `t('common.close.aria', resolveDomLang())`, not upstream's hardcoded English `Close`. | GAP-02 / D-38-09..D-38-11: this is a French-default product whose shared dialog primitive announced an English accessible name to every screen-reader user in both languages. `src/components/ui/**` is excluded from ESLint, so the `no-restricted-syntax` guard that flags hardcoded JSXText does not see this file — exactly where the defect lived. `tests/dialog-close-label.test.ts` catches it, but only after the fact. Added 2026-09-06 (38-01). |
| `src/components/ui/sheet.tsx` | The icon-only close button's `sr-only` label reads `t('common.close.aria', resolveDomLang())`, not upstream's hardcoded English `Close`. | GAP-02 / D-38-09..D-38-11: identical defect and fix to the `dialog.tsx` row above — both files vendor the same close-button chrome. `src/components/ui/**` is excluded from ESLint, so the `no-restricted-syntax` guard that flags hardcoded JSXText does not see this file. `tests/dialog-close-label.test.ts` catches it, but only after the fact. Added 2026-09-06 (38-01). |
