...
| `password authentication failed (SuperSecretPw123)` | unchanged | `…([redacted])` via the env scrub |
...
- **IN-02** — uppercase hostnames refused. **Incidentally resolved** by CR-01, exactly as the
  review anticipated ("already folded into the CR-01 fix"). Demonstrated above:
  `DEV-PROBE.INVALID` → `dev-probe.invalid`. No separate commit.
