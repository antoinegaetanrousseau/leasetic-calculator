...
status: executing
last_updated: "2026-09-09T22:40:47.278Z"
...
| @react-pdf/renderer 4.5.1 exact-pin (no caret) — first PDF library install in the project; React 19 peer dep range includes ^19.0.0 (no --legacy-peer-deps needed) | 08-05 execution | 08-05 |
| Font format for @react-pdf/renderer changed from woff2 to TTF — woff2 fails fontkit TTFSubset with DataView bounds error when subsetting multi-weight fonts with accented glyphs (fontkit upstream bug). Fix: wawoff2.decompress() + Buffer.from(uint8) correct-slice write. TTF bytes derived from same source woff2. Determinism preserved (PROP-19 satisfied). | Rule 1 auto-fix | 08-05 |
...
| Phase 14 schema dichotomy: NEW `createPartnerFormSchema` (7 fields) kept distinct from legacy `createPartnerSchema` (3 fields). Legacy modal (D-10 shelf code) imports the original; the new /partners/new form imports the new one. +44 LOC over extending in-place but eliminates drift risk against the shelved modal | 14-02 design call | 14-02 |
| Phase 14 audit-log `profile` sub-key envelope: extended PII fields (firstName/lastName/companyName/siret/phone/invitationMessage) persist as a single `profile` sub-key inside the existing `user.create` + `invitation.create` payloads — empty-string values dropped, key omitted entirely when all extended fields are empty. Preserves the legacy 3-field payload shape exactly for queries that pre-date Phase 14. ADMIN-09 D-09-09b redaction comment EXTENDED (not weakened): the profile fields are PII not commission/rate | 14-02 design call + ADMIN-09 preservation | 14-02 |
...
| Single-active row expand on `/history` (D-25): `useState<string \| null>(null)` lifted to `<CoefficientHistoryList>` (parent), passed to each row as `isExpanded` + `onToggle`. Toggle uses `prev => prev === row.id ? null : row.id` — opening row B sets expandedRowId=row-B.id which makes row-A.isExpanded evaluate false. Contrasts with sidebar's row-local useState (multi-expand per D-20). Sidebar tolerates per-row state because row count is bounded at 5; /history may have 20+ rows where single-active matches §5.3 vertical-rhythm needs | 14-05 D-25 implementation pattern | 14-05 |
...
- **2026-05-20:** 14-02 executed — /partners/new route + 3-section RHF+zod form + extended adminCreateInvitation. Task 1 (TDD): added `createPartnerFormSchema` (7 fields per UI-SPEC §5.1) alongside untouched legacy `createPartnerSchema` (D-10 shelf code preserved); extended `adminCreateInvitation` to accept 6 optional PII fields persisted under an audit_log payload.profile sub-key with empty-string drop discipline. ADMIN-09 D-09-09b redaction comment extended (not weakened). 9 tests added (5 schema + 4 action). Task 2 (TDD): server-component page.tsx (requireAdmin + dynamic='force-dynamic' + robots:noindex) + client CreatePartnerForm.tsx (RHF onBlur, 3 ●-sections separated by `<hr>`, action bar with cancel ghost-link + submit btn-green spinner, useWatch-driven char counter with --danger color above 1000 chars) + `createPartnerInvitationAction` wrapper. 28 partners.new.* i18n keys × FR/EN + 1 error.field.siret.invalid × FR/EN (Rule 3 auto-fix — UI-SPEC §6.4 listed it as 'existing' but missing). 7 tests added (2 page + 5 form). 3 auto-fix deviations logged (missing SIRET key, watch→useWatch lint, server-only chain in client bundle); 1 plan-vs-shipped-contract reconciliation (kind:'invite' not 'invitation'); 1 UI-SPEC §5.1.3 PhoneInput recommendation declined (international-partner support). Total test delta: 789 → 805 (+16). 5 task commits: f8987ec (RED Task 1), 0cbf862 (GREEN Task 1), 5bab79d (RED Task 2), 64a9de1 (GREEN Task 2 route+form), 55550d1 (i18n).
...
- **2026-05-12:** 13-05 executed — Step-3 verification route: `app/(authed)/proposals/new/verification/page.tsx` (357 lines) + colocated `page.test.tsx` (489 lines, 15 Vitest assertions) + `FinalizeButton.tsx` (112 lines) + colocated `FinalizeButton.test.tsx` (301 lines, 8 Vitest assertions). Server component: requireUser() first; D-03 silent self-heal × 5 redirect paths (no draft_id / null draft / non-draft or deletedAt / incomplete-inputs proposalInputSchema.safeParse fail per UI-SPEC §10.3 — no visual fallback on review surface / missing-globalParams); server-side compute via getLatestGlobalParams + computeLoyer (mirroring plan 13-04); 2-column 1040px grid per UI-SPEC §5.7 (`grid-template-columns: minmax(0, 1fr) 360px`, `gap: 24px`); left column: 3 RecapSection cards (CLIENT/PROJET/CALCUL) with ← Modifier links — CLIENT+PROJET → /parametres, CALCUL → /calcul; D-25 hide-when-empty for accordion-field rows (clientRole / clientSiren / projectDesc) per UI-SPEC §16 OQ#5; right column: PdfPreviewMock with D-15 literal LC-2026-XXX placeholder (sourced from i18n key, never hardcoded in JSX); WizardActionBar via FinalizeButton 'use client' adapter at the bottom (outside the grid, full 1040px width). FinalizeButton: useState(isSubmitting) + fetch POST `/api/proposals/finalize` with `{ draftId }` + Content-Type: application/json; on 200 success → toast.success(wizard.toast.finalize.success) + router.push(`/proposals/${data.id}`); intentionally does NOT reset isSubmitting on success (redirect unmounts; prevents double-click race); on non-OK / network error → toast.error with duration: 5000 + setIsSubmitting(false) for retry; bounded error disclosure (never echoes response body). D-12 ADMIN-09 partial relaxation EXTENDED to step 3: commission rendered EXACTLY ONCE in ● CALCUL recap row 2 with `(non visible client)` sub-line; absent from PdfPreviewMock (no commission prop, structurally locked by plan 13-01 §5.3), CLIENT recap, PROJET recap, hidden inputs, data-* attributes, and the POST payload. Test 15 enforces EXACTLY-ONCE visibility + DOM walks for hidden inputs + data-* attributes. Test 14 enforces ROUTE-01 SC5 (cross-user redirect, no leak). typecheck 0; lint 0 errors (3 pre-existing warnings unrelated); 23/23 plan tests + 743/743 full suite (no regression); 11/11 grep contracts pass (1 PdfPreviewMock JSX mount, 3 RecapSection JSX mounts, 5 redirect-to-parametres paths, 3 // D-12 annotations, 2 "1040" max-width refs, 1 api/proposals/finalize fetch, 2 toast.success+toast.error sites). 1 small clarification (PLAN.md `<action>` showed `replace('← ', '')` on the dictionary label — implementation passes literal `'Modifier'` because RecapSection prepends `← ` itself; matches plan 13-04's idiom; behavior unchanged). ROUTE-01 Wave 2 complete (step-3 surface inventory shipped — combined with plans 13-03/04 + plan 13-02 + plan 13-01, ROUTE-01's 5 success criteria are structurally satisfied; plan 13-06 closes the requirement with the integration suite + D-28 STRIDE addendum + golden-PDF byte-determinism gate). 4 task commits: ba2e8a6 (test RED Task 1), 047c833 (feat GREEN Task 1 FinalizeButton), f3db401 (test RED Task 2), 23517a3 (feat GREEN Task 2 verification page).
...
- **2026-05-20:** 14-05 executed — Standalone `/[adminSegment]/history` route shipped (ROUTE-02 SC #5). Task 1 (TDD RED→GREEN): page.tsx server component (requireAdmin defence-in-depth + force-dynamic + cursor extraction via Phase 12's decodeCoefficientHistoryCursor with malformed-fallthrough mitigation for T-14-05-02 + listCoefficientHistory({ cursor?, limit: 20 }) + encode next + h1/subtitle hero); CoefficientHistoryList.tsx ('use client', single-active expandedRowId state per D-25, document-level Escape listener guarded by `if (expandedRowId === null) return` + cleanup, pagination footer with conditional ← Page précédente (when currentCursor !== null, routes to bare /<seg>/history per planner recommendation) + Page suivante → links, empty-state card branch handling rows=[] + currentCursor=null AND rows=[] + currentCursor !== null past-last-page); CoefficientHistoryRow.tsx ('use client', stateless wrt expansion, <article aria-labelledby> card with --teal border when expanded, trigger button with aria-expanded + aria-controls + label morph 'Voir le détail →' ↔ '↓ Masquer le détail', focus management via useRef + useEffect transition detection + requestAnimationFrame for collapse-side trigger refocus, mounts <CoefficientDiffPanel mode="full" onClose={onToggle}> when expanded). 8 NEW history.* i18n keys × FR + EN (16 entries) per UI-SPEC §6.6 (history.title, history.subtitle, history.row.viewDetail, history.row.hideDetail, history.diff.summary.label, history.pagination.next, history.pagination.previous, history.empty). Test count delta: 830 → 847 (+17): 4 page (auth gate + cursor decode + list-mount props + header copy) + 13 list (3-row mount + single-active D-25 + Escape collapse + diff panel full-mode mount + onClose wiring + next/prev link conditional rendering + empty-state both branches + i18n parity + meta render + trigger label morph). typecheck 0; lint 0 errors (3 pre-existing Phase 12 warnings unchanged); build 0 — `/[adminSegment]/history` appears as ƒ (dynamic) in route manifest. ZERO deviations — plan executed exactly as written. 3 deferred polish items documented in SUMMARY: explicit previous-cursor encoding (v1.3+), CSS :hover border-color (v1.3+), on-expand focus to Fermer × (v1.3+ — requires Plan 14-04 panel ref-forwarding). D-30 admin-only commission visibility preserved: collapsed-row HTML may render commission transitions via row.summary (T-14-05-03 mitigate via requireAdmin gate); expanded-row diff panel renders commission_pct (T-14-05-04 accept, locked by Plan 14-04 Test 8). Plan 14-06 grep gate will cover the strict D-29 partner-list surface (different surface, unaffected by this plan). 2 task commits: 1baf55c (test RED), 45ed2a9 (feat GREEN). ROUTE-02 success criterion #5 grounded.

## Open Blockers

None — Plan 08-05 complete. Plans 08-06 (CI byte-determinism gate) and 08-07 (POST handler) are unblocked.
...
| Phase 44 P03 | ~9min | 2 tasks | 4 files |
| Phase 44 P04 | ~12min | 2 tasks | 4 files |
...
- [Phase ?]: Plan 18-05 — Open Q9: admin.coefficients.page.title value shortened FR+EN to 'Coefficients' (single consumer verified)
...
- [Phase 44]: A 'migrated' RowOutcome projects to outcome: 'rendered' in the backfill dry-run report — the report describes what was/would be re-rendered, not which mode produced the row — 44-03: dry-run and apply share one report shape
- [Phase 44]: computeBackfillDrift classifies a stored-report id missing from the fresh set as 'alreadyMigrated' (not drift) when it is in the migratedIds marker set — 44-03/D-09/MIG-05: a resumed apply must never abort on its own completed rows
- [Phase 44]: runBackfill mirrors reconcile/run.ts's gate-then-loop shape: three apply-mode refusal gates (no report, fingerprint mismatch, drift) all return before the single write-authorising loop (D-08, D-09)
- [Phase 44]: live-deps.ts is the sole module in src/lib/backfill/ importing @/lib/db, @/lib/storage and @/lib/pdf at runtime, keeping run.ts/render-row.ts/pdf-data.ts/report.ts/drift.ts unit-testable with no runtime-guard shim
