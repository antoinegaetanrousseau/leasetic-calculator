...
- [x] **ROUTE-02**: System exposes a dedicated route `/[adminSegment]/partners/new` that replaces v1.1's modal-based partner creation. Form contains three sections — personal info (prénom, nom, email), company info (société, SIRET optionnel, téléphone), and a customizable invitation message. Submitting issues the same one-time-URL invitation as v1.1. **Success criterion #3 partner-home MetricTiles DEFERRED to v1.3 (see PROJECT.md decisions log)**. Color-contrast HUMAN-VERIFY checkpoint on Plan 14-06 Task 4 still pending — diff-panel changed-row pill composite (rgba(224,133,48,0.10) over --surface with --ink weight 600) needs WCAG 2.1 AA verification in both light and dark themes.
...
| ROUTE-02 | Phase 14 | Structurally Complete (all 6 Plans 14-01..14-06 shipped — `/[adminSegment]/partners/new` route + 3-section RHF form + 28 partners.new.* keys (criterion #1); admin home rebuilt with 3 AdminNavCards + 9 admin.{home,nav}.* keys (criterion #3 admin portion; partner-home MetricTiles DEFERRED to v1.3); `/coefficients` 2-col layout + CoefficientHistorySidebar (5 recent edits) with inline-expansion CoefficientDiffPanel mode="condensed" + standalone `/history` route with cursor pagination + single-active expand (criterion #5); Plan 14-06 closed criteria #2 + #4: AccountsList CTA now `<Link href=\`/${adminSegment}/partners/new\`>` per D-11 (modal preserved as D-10 shelf code); StatusChip invited variant rendered on partner list per D-26; ProposalRow + proposal detail header rendered via deriveDisplayStatus per D-27 + D-28; tests/admin-09-grep-contracts.test.ts (9 D-29 grep gates) blocks any regression across the 4 non-exempt Phase 14 surfaces. Total Phase 14 Vitest delta: 788 → 872 (+84 tests). HUMAN-VERIFY checkpoint on Plan 14-06 Task 4 still pending — diff-panel changed-row pill composite (rgba(224,133,48,0.10) over --surface with --ink weight 600) needs WCAG 2.1 AA measurement in both light and dark themes) |
