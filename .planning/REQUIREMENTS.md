...
| ROUTE-02 | Phase 14 | In Progress (Plan 14-01 foundation + Plan 14-02 route + Plan 14-03 admin home shipped — `/[adminSegment]/partners/new` server-component route + 3-section RHF form + adminCreateInvitation extended with PII `profile` sub-key + 28 partners.new.* i18n keys (criterion #1 met); admin home rebuilt with 3 `<AdminNavCard>` instances (Sliders/Users/History) per D-13..D-16 + 9 admin.{home,nav}.* i18n keys × FR+EN (criterion #3 admin-home portion met — partner-home MetricTiles deferred to v1.3 per CONTEXT.md). Plan 14-06 will satisfy criterion #2 by swapping AccountsList's "Créer un partenaire" CTA from `<button onClick={openModal}>` to `<Link href=\`/${adminSegment}/partners/new\`>` per D-11) |
