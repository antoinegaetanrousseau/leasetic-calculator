...
| ROUTE-02 | Phase 14 | In Progress (Plans 14-01..14-05 shipped — `/[adminSegment]/partners/new` server-component route + 3-section RHF form + 28 partners.new.* i18n keys (criterion #1 met); admin home rebuilt with 3 `<AdminNavCard>` instances + 9 admin.{home,nav}.* i18n keys (criterion #3 admin-home portion met — partner-home MetricTiles deferred to v1.3); `/coefficients` converted to 2-col layout (1040px / 360px sidebar) with `<CoefficientHistorySidebar>` showing 5 most-recent edits + inline-expansion shared `<CoefficientDiffPanel mode="condensed">` (10 new coefficients.history.* + history.diff.* i18n keys) — criterion #5 sidebar portion met; standalone `/[adminSegment]/history` route shipped via Plan 14-05 with cursor pagination (20/page via Phase 12 encode/decode helpers), single-active row expand per D-25 owned by `<CoefficientHistoryList>`, consumes `<CoefficientDiffPanel mode="full">` from Plan 14-04, 8 new history.* i18n keys (criterion #5 standalone-route portion now met). Plan 14-06 will satisfy criterion #2 by swapping AccountsList's "Créer un partenaire" CTA from `<button onClick={openModal}>` to `<Link href=\`/${adminSegment}/partners/new\`>` per D-11, ship the StatusChip invited variant per D-26 + D-27 + D-28, and install ADMIN-09 D-29 grep-contract test suite) |
