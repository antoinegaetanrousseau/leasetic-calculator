...
### ✅ v1.7 — Sales Motivation (Phase 35) — SHIPPED 2026-09-05
...
<details>
<summary>✅ v1.8 — Deferred Items (Phases 36-40) — SHIPPED 2026-09-07</summary>
...
- [x] 35-04-PLAN.md — mutation-verified owner-isolation and week-boundary proof against real Postgres, skip-by-default on `DATABASE_URL_TEST` (wave 3)
- [x] 35-05-PLAN.md — home-page wiring with the admin gate at the call site, updated page tests, and human verification of the rendered surface (wave 4)
...
*Phase details for v1.8 (Phases 36-40) are archived in `milestones/v1.8-ROADMAP.md`.*
...
- [x] 42-02-PLAN.md — users.telephone + users.company_telephone + leasetic_advisor table, migration 0011, operator MIGRATE PROD checkpoint

**Wave 2** *(blocked on Wave 1)*

- [x] 42-03-PLAN.md — clientSiret with the SIREN-prefix refine, optional partnerTel, and siege.siret in the registry parser
- [x] 42-04-PLAN.md — Better Auth additionalFields registration + /parametres telephone and read-only fonction
- [x] 42-05-PLAN.md — Admin partner form: company telephone persistence and the new partner telephone field
- [x] 42-06-PLAN.md — Advisor singleton persistence: fixed-id query helpers, form schema, admin-gated action
...
- [x] 42-09-PLAN.md — Session hydration of partnerTel into draft inputs and clientSiret resume prefill
- [x] 42-10-PLAN.md — Finalize telephone gate, legacy-draft pre-check, two bounded codes, and the FinalizeButton dialog
...
- [x] 43-01-PLAN.md — Reconcile DOC-03 upstream (D-03) and transcribe the design's ten-step scale, palette and mm page margins into `styles.ts`

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 43-02-PLAN.md — Port the lockup and icon marks to `<Svg>` primitives, evidence-first, with the rotate-transform pre-applied (D-07)
- [x] 43-03-PLAN.md — Add the design's full FR/EN `pdf.*` copy and ledger the keys the redesign orphans for deletion alongside their last consumer
- [x] 43-04-PLAN.md — Grow `ProposalDocumentProps` and thread the advisor + partner phone through BOTH render call sites; add the `emDash` formatter (D-12/D-13)

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 43-05-PLAN.md — Rebuild the page frame, lockup header, 2px rule, 21pt title + pills and the two cards; delete the three keys it retires (DOC-01/02/03)
...
- [x] 43-07-PLAN.md — Prove DOC-10/11/12/13 + FIELD-03 against rendered bytes, reconcile the Phase 41 four-faces proof, and regenerate the fixture baseline (D-16)
...
- [x] 43-09-PLAN.md — Disable react-pdf's default hyphenator and convert `dehyphenate()` from a normaliser into a DOC-01 guard (Gap 1)

**Wave 8** *(blocked on Wave 7 completion)*

- [x] 43-10-PLAN.md — Rebuild the VOTRE CONTACT card to Antoine's 7-row structure; delete `salesRep`/`advisorName`, add `pdf.card.contact.partner` (Gap 2)

**Wave 9** *(blocked on Wave 8 completion)*

- [x] 43-11-PLAN.md — Finding 3, operator option (a): `partnerCo` sourced from `companyName` alone at both hydration sites, absence rendering an em dash

**Wave 10** *(blocked on Wave 9 completion)*

- [x] 43-12-PLAN.md — Add `clientSiret` to the happy-path fixtures with a positive render assertion, and regenerate the byte-determinism baseline ONCE (Gap 3, D-16)

**Wave 11** *(blocked on Wave 10 completion)*

- [x] 43-13-PLAN.md — Operator confirmation on ONE real finalized proposal that a captured SIRET prints (checkpoint, human-only)
...
- [x] 44-01-PLAN.md — Contracts & data layer: the `proposal.pdf_backfill` audit action, the anti-join candidate query, and `src/lib/backfill/types.ts`

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 44-02-PLAN.md — Per-row render: stored row -> document props with verbatim `computed` and its own `language`, then mode-aware upload/persist/mark
- [x] 44-03-PLAN.md — Two-form dry-run report and the three-way drift gate

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 44-04-PLAN.md — The `runBackfill` orchestrator, its refusal gates, and the production dependency wiring
...
| 44. Backfill Migration | v1.9 | 4/5 | In Progress|  |
