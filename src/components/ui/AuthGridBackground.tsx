...
// NOTE: the auth-1 block shipped this file WITHOUT a 'use client'
// directive — it relied on auth.tsx, its only importer, carrying one.
// We import it straight into the (public) Server Component layout, so it
// needs its own or the hooks below fail the production build.
/**
 * Animated grid backdrop for the public/auth pages.
 *
 * Adopted from the ReUI Pro block `auth-1` (see docs/design/reui-blocks-audit.md).
 * Taken as-is apart from this note: it depends only on `motion` and `cn`, and
 * `excludeSelector` keeps the animated cells clear of whatever carries
 * `data-auth-surface` — the form card — so squares never flicker behind the
 * inputs.
 *
 * auth-1's other half is a split-screen editorial photo panel hot-linking two
 * Unsplash images. That was NOT adopted: we have no such asset, and a
 * half-empty right column reads worse than a centred card. The grid is the part
 * of the block that carries its character without needing artwork.
 */
import {
  useCallback,
  useEffect,
